...
class StatscomidaController extends AbstractController
{
    ...
    /**
     * @Route("/statscomida", name="statscomida")
     */
    public function index(): Response
    {

        $allComida = $this->entityManager->getRepository(Comida::class)->findAll();

        $data_all = [];

        foreach ($allComida as $comida) {
            if ($comida->isIsComida()) {
                $temp = [
                    $comida->getName() => $comida->getId(),
        ...
        $topPlatos = $this->statsRepository->findTop5PlatosLastWeek();
        ...
        $allFoodRepetitions = $this->statsRepository->allFoodRepetitions();
        $data = array();
        foreach($allFoodRepetitions as $all){
        ...
        return $this->render('statscomida/index.html.twig', [
            'chartData' => json_encode($chartData),
            'chartDataMonth' => json_encode($chartDataMonth),
            'chartWeek' => json_encode($chartWeek),
            'allcomida' => $data_all,
            'allcomidarepetitions' => $data
        ]);
    }

    /**
     * @Route("/comidasemana", name="comida_semana")
     */
    function buscar(Request $request): Response
    {
        ...
        $platoSemana = $this->statsRepository->findDishLastWeek($idComida);
    ...
    /**
     * @Route("/restablecersemana", name="restablecer_semana")
     */
    function restablecerSemana(Request $request): Response
    {
        ...
        $platoSemana = $this->statsRepository->findTop5PlatosLastWeek();
